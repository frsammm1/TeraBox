...
export class TransferManager implements TransferManagerLike {
  ...
  async enqueue(request: TransferRequest): Promise<TransferSnapshot> {
    ...
    if (
      ...
      throw new TransferError("Only one active or queued upload is allowed per chat. Use /jobs or /cancel first.");
    }
  ...
  private async execute(job: InternalJob): Promise<void> {
    try {
      ...
      if (file.sizeBytes > this.config.maxSourceBytes) {
        throw new TransferError(
          `File is ${formatBytes(file.sizeBytes)}. Server limit is ${formatBytes(this.config.maxSourceBytes)}.`,
          false,
        );
      }
      ...
      if (requiresSplit && !this.config.splitOversizeFiles) {
        throw new TransferError(
          `File is larger than ${formatBytes(this.config.maxUploadBytes)}. ZIP splitting is disabled.`,
          false,
        );
      }
      ...
      const sourcePath = path.join(job.workspaceDirectory, job.filename);
      ...
      if (!requiresSplit) {
        ...
        const parts = await this.splitter.split({
          ...
          outputDir: job.workspaceDirectory,
