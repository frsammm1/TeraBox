...
function helpMessage(uploadEnabled: boolean): string {
  return [
    ...
    ...(uploadEnabled
      ? [
          "• <code>/upload &lt;TeraBox URL&gt; [file-number]</code> — Telegram par upload queue karein",
...
export class TelegramBot {
  ...
  private async handleUpdate(update: TelegramUpdate): Promise<void> {
    ...
    if (!this.isAllowed(message.from.id)) {
      await this.sendText(message, "⛔ This bot is restricted to authorized users.");
      return;
    }
  ...
  private async queueUpload(
    message: TelegramMessage,
    selection: Pick<TransferRequest, "surl" | "directory" | "fileIndex" | "ownerUserId">,
  ): Promise<void> {
    if (!this.transferManager) {
  ...
  private isAllowed(userId: number): boolean {
    return this.config.allowPublic || this.config.allowedUserIds.has(userId);
  }
