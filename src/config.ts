...
function loadTransferConfig(env: NodeJS.ProcessEnv, telegram: TelegramConfig | undefined): TransferConfig | undefined {
  ...
  return {
    ...
    downloadChunks: readInteger(env, "TRANSFER_DOWNLOAD_CHUNKS", 8, 1, 32),
  };
}
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): AppConfig {
  ...
  if (token) {
    ...
    if (!allowPublic && allowedUserIds.size === 0) {
      throw new Error(
        "Set TELEGRAM_ALLOWED_USER_IDS to your numeric Telegram user ID, or explicitly set TELEGRAM_ALLOW_PUBLIC=true.",
      );
    }
